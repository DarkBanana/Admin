      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <div class="row-fluid">
    <div id="dataOption" class="pull-right">
        <div class="btn-group">
            <button  type="button" id="adduser" @click="adduser" class="btn btn-danger btn-sm" style="height:25px ;width:68px;">
...
<script>
	...
	var table = $('#dataTable').DataTable( {
	 ...
	 $('#dataTable tbody').on( 'click', 'button#delRow', function () {
	   ...
	   //修改方法必须放在$datatable下 否则无法绑定default的button 不好用vue
	    ...
	    btn: ['修改','取消'],
	        btn1: function (index, layero) {
			    ...
			    var de =  "&id="+data.id+"&useraccount="+useraccount+"&username="+username
						  ...
						  +"&worktype"+worktype+"&position"+position+"&note="+note;							
				console.log(de)
				$.ajax({
						type: "POST",
	  ...
	  $.ajax({
			...
			var data = JSON.parse(data);
		 	 for (var i in data) {  
               ...
               $("#role").html("<option value='请选择'>--选择角色--</option> "+optionstring);  
             }  
		layer.open({
		    ...
		    btn: ['添加','取消'],
		        btn1: function (index) {
					var data =  "useraccount="+vm.useraccount+"&username="+vm.username
								...
								+"&worktype="+vm.worktype+"&role="+vm.role
